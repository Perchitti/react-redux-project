...
export const fetchPosts = () => dispatch => {
  fetch("https://localhost:3001/posts")
    ...
    .then(posts =>
      dispatch({
        //we are dispatching the data to the reducer, this calls on the reducer, it jumps to postReducer
        type: FETCH_POSTS,
...
export const createPost = postData => dispatch => {
  //postData because we're submitting the post
  fetch("https://jsonplaceholder.typicode.com/posts", {
    ...
    .then((
      post //then we want the appropriate data back
    ) =>
      dispatch({
        type: NEW_POST,
        payload: post
